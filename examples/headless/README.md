...
| --- | --- |
| `0` | 演示已得到终态结果，或成功打印帮助 |
| `1` | 预设输入意外返回 `Unhandled`，或脚本结束时没有终态结果 |
...
[演示库](src/lib.rs) 导出 `DemoRule`、`DemoVariant`、`DemoPhase`、`DemoInput`、`DemoEvent`、`DemoOutput` 和 `DemoMachine`。其他宿主可以通过 `Engine::<DemoVariant>::new(DemoRule::new(seed, align_east)).into_machine()` 创建机器，直接使用原生 `dispatch()`、`state()` 和 `state().output()`。
